    ...
    <table [ngStyle]="{width:(tableWidth+'px')}" *ngIf="!flat">
        ...
        <tr>
          <th *ngFor="let item of headers">
            ...
            <div *ngIf="item.headerTemplate">
              <ng-container
                *ngTemplateOutlet="item.headerTemplate;context: {$implicit: item, header:item, prop:item.prop};">
              </ng-container>
            </div>
          </th>
        </tr>
      ...
      <tbody>
        <tr *ngFor="let row of list;let index=index;">
          <td *ngFor="let item of headers;let n=index;" #td [class]="item.css"
            [attr.rowspan]="calcRowspan&&calcRowspan(index,n,td)" 
            [attr.colspan]="calcColspan&&calcColspan(index,n,td)"
            [ngStyle]="{
              'max-width':item.width+'px'
            }">
            <div *ngIf="!item.useTemplate&&!templates[item.prop]" [title]="row[item.prop]||''">{{row[item.prop]||''}}
            </div>
            <div *ngIf="item.useTemplate||templates[item.prop]">
              <ng-container *ngTemplateOutlet="item.useTemplate||templates[item.prop];
              context: {
                $implicit: row, 
                row: row,
                value:row[item.prop],
                index:index,
                prop:item.prop,
                tdData:getTdData(index,n)
              };">
              </ng-container>
            </div>
          </td>
        </tr>
        <tr class="empty-list" *ngIf="list?.length==0&&loaded">
          <td [attr.colspan]="headers?.length">
            <svg viewBox="0 0 1567 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
...
<div class="flat-list-container" *ngIf="flat">
  <div *ngFor="let row of list;let index=index;">
    <ng-container *ngTemplateOutlet="contentTemplates?.first;context: {$implicit:row,row:row,index:index};">
    </ng-container>
  </div>
</div>
<!-- 分页 -->
<div class="page-navigator" *ngIf="totalSize">
  ...
  <div class="page-no">
    ...
    <span (click)="toPage(pageIndex+1)"
      *ngIf="pageIndex+1<pageTotalIndex&&(pageIndex+1!=(pageToalIndex-3))">{{pageIndex+1}}</span>
    <span (click)="toPage(pageIndex+2)"
      *ngIf="pageIndex+2<pageTotalIndex&&(pageIndex+2!=(pageToalIndex-2))">{{pageIndex+2}}</span>
...
<ng-template let-item #treetableTmp>
  <div class="node-deep" [ngStyle]="treetableInstance.calcDeepStyle(item)">
    <span class="fa" [class.fa-square-o]="!item.children?.length"
      [class.fa-plus-square-o]="item.children?.length&&!item.$$expand"
      [class.fa-minus-square-o]="item.children?.length&&item.$$expand"
      (click)="item.$$expand?treetableInstance.compress(item):treetableInstance.expand(item)">
    </span>
  </div>
</ng-template>
